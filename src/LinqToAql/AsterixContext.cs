...
namespace LinqToAql
{
    ...
    public abstract class AsterixContext
    {
        ...
        {
            foreach (FieldInfo field in GetType().GetTypeInfo()
                .GetFields(BindingFlags.Public | BindingFlags.Instance)
                .Where(f => f.GetValue(this) == null && f.FieldType.GetTypeInfo().IsGenericType))
            {
                var dataset = GetDatasetForType(field.FieldType);
                if (dataset != null)
                    field.SetValue(this, dataset);
            }
            foreach (PropertyInfo prop in GetType().GetTypeInfo()
                .GetProperties(BindingFlags.Public | BindingFlags.Instance)
                .Where(p => p.CanWrite && p.GetValue(this) == null && p.PropertyType.GetTypeInfo().IsGenericType))
            {
