    ...
    internal class Join : AqlFunctionVisitor
    {
        ...
        public override bool IsVisitable(MethodCallExpression expression)
        {
            return
                new[]
                {
                    typeof(string).GetTypeInfo().GetMethod("Join", new[] { typeof(string), typeof(IEnumerable<string>) }),
                    typeof(string).GetTypeInfo().GetMethod("Join", new[] { typeof(string), typeof(string[]) })
                }
