...
namespace LINQToAQL
{
    ...
    public class SampleQueryExecutor : IQueryExecutor
    {
        ...
        public IEnumerable<T> ExecuteCollection<T>(QueryModel queryModel)
        {
            ...
            ParameterExpression currentItemProperty = Expression.Parameter(typeof (SampleDataSourceItem));
            Expression<Func<SampleDataSourceItem, T>> projection =
                Expression.Lambda<Func<SampleDataSourceItem, T>>(queryModel.SelectClause.Selector,
                    currentItemProperty);
            Func<SampleDataSourceItem, T> projector = projection.Compile();
